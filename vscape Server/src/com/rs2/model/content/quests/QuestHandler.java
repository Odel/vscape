...
public class QuestHandler {
	...
	public static final int[] QUEST_IDS = { 8145, 8146, 8147, 8148, 8149, 8150,
	...
	private static Quest[] quests = { new CooksAssistant(),
			new TheKnightsSword(), new TheRestlessGhost(), new DoricsQuest(),
			new ImpCatcher(), new RuneMysteries(), new WitchsPotion(),
			new EasterEvent(), new SheepShearer(), new PrinceAli(),
			new DruidicRitual(), new MerlinsCrystal(), new ElementalWorkshop(),
			new ShieldOfArrav(), new LostCity(), new DragonSlayer(),
			new RomeoAndJuliet(), new DemonSlayer(),
			new BlackKnightsFortress(), new GoblinDiplomacy(),
			new PiratesTreasure(), new VampireSlayer(), new ErnestTheChicken(),
			new PriestInPeril(), new GhostsAhoy(), new AnimalMagnetism(),
			new HorrorFromTheDeep(), new HeroesQuest(), new FamilyCrest(),
			new TheGrandTree(), new DwarfCannon(), new WaterfallQuest(),
			new GertrudesCat(), new TreeGnomeVillage(), new ChristmasEvent(),
			new RecruitmentDrive(), new MonkeyMadness(), new NatureSpirit(),
	 ...
	 * player.setQuestsLength(quests.length); for(Quest q : quests) {
	 * player.setQuestStage(q.getQuestID(), 0); } player.sendQuestTab(); //Makes
	 * the quest log empty except implemented quests
	 * PlayerSave.loadQuests(player); //loads quest progress from Username.txt,
	 * sets variables }
	 */
